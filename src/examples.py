...
from src.yandex_disk_api import YandexDiskRestClient


class ExamplesOfUsingOfYandexDiskRestClient:
    def __init__(self):
        token = "AQldKZoAAxRsgJCGXLoYTBqa-oCC0LAQvQ"
    ...
    def remove_folder_or_file(self):
        try:
            ...
            print(
                "The folder " + self.random_name_of_folder + " was successfully removed.")
        except YandexDiskException as exp:
            print(exp)
            sys.exit(1)
    ...
    def get_list_of_all_files(self):
        try:
            ...
            print(
                "There are " + str(len(files)) + " files in this Yandex.Disk")
        except YandexDiskException as exp:
            print(exp)
            sys.exit(1)
    ...
    def upload_file_from_url(self):
        try:
            ...
            self.client.upload_file_from_url(url,
                                             self.random_name_of_parent_folder + "/jeett_dt_001.png")
            print(
                "File jeett_dt_001.png was downloaded to the folder " + self.random_name_of_parent_folder)
        except YandexDiskException as exp:
            print(exp)
            sys.exit(1)
...
def main():
    ...
    #examples.run()
    client = YandexDiskRestClient(token = "fg")
