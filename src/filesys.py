...
class FileSystemImage:
    ...
    def sync_local_priority(self, remove=True, save_orig=False, mes=False):
        self._sync_local_priority(self.local, self.disk, remove, save_orig,
                                  mes)

    def _sync_local_priority(self, local, disk, remove, save_orig, mes):
        if local['type'] == disk['type'] == "file":
            if local['name'] == disk['name']:
                if local['md5'] != disk['md5']:
                    if save_orig:
